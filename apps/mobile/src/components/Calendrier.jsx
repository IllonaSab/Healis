...
// Formate un objet Date en chaîne "AAAA-MM-JJ"
function dateToStr(date) {
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
}

// Calcule les 7 jours de la semaine courante en démarrant toujours du Lundi
function getWeekDays(referenceDate) {
  const date = new Date(referenceDate);
  const day = date.getDay();

  const monday = new Date(date);
  // Formule pour reculer jusqu'au lundi précédent (dimanche = 0, donc (0+6)%7 = 6 jours en arrière)
  ...
  return Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      date: d.getDate(),             // Numéro du jour (1-31)
      str: dateToStr(d),             // Clé unique de la date "AAAA-MM-JJ"
    };
  });
}

const CURRENT_YEAR = new Date().getFullYear();
// Liste déroulante des années (-5 ans à +4 ans par rapport à l'année actuelle)
...
export default function Calendrier({ onSelectDay }) {
  ...
  // Nombre de semaines de décalage par rapport à aujourd'hui (-1: semaine passée, +1: suivante)
  ...
  // Date pivot calculée à partir du décalage de semaines
  const referenceDate = new Date(today);
  referenceDate.setDate(today.getDate() + weekOffset * 7);

  const days = getWeekDays(referenceDate);

  // Titre du mois basé sur le jeudi (milieu de semaine) pour éviter les incohérences en fin de mois
  ...
  // Sélection d'un jour au clic sur la pastille et envoi de la date au composant parent (Dashboard)
  const handleSelect = (day) => {
    setSelectedStr(day.str);
    onSelectDay?.(day.str);
  };

  // Convertit le saut vers un mois/année précis en nombre de semaines de décalage
  ...
  return (
    <View style={styles.container}>
      {/* En-tête : Ouvre la modale de sélection de date rapide */}
          ...
          {days.map((day, i) => {
            ...
            return (
              <View key={i} style={styles.dayColumn}>
                ...
                {/* Pastille circulaire avec styles distincts pour aujourd'hui et le jour sélectionné */}
                <TouchableOpacity
                  style={[
                    styles.dateCircle,
      ...
      {/* Modale de saut de date rapide par Mois et Année */}
